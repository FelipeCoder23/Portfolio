...
def non_max_suppression(
    ...
    # Imprimir el valor de 'mi'
    print(f"Valor de mi: {mi}")

    # Imprimir la forma del tensor 'prediction' para entender su estructura
    print(f"Forma de prediction: {prediction.shape}")

    # Si 'mi' es mayor que 4, imprimir la forma del rango seleccionado para confirmar que no es vacío
    if mi > 4:
        print(f"Forma de prediction[:, 4:mi]: {prediction[:, 4:mi].shape}")
    else:
        print("El rango seleccionado de 'prediction[:, 4:mi]' es probablemente vacío debido a que 'mi' es menor o igual a 4.")

    # Ahora la línea problemática (para referencia, no necesitas agregarla de nuevo)
