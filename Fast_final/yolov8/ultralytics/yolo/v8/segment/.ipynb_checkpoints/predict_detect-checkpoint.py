...
class DetectionPredictor(BasePredictor):
    def __init__(self, cfg):
        super().__init__(cfg)
        self.detections = []  # Inicializar la lista de detecciones
    ...
    def write_results(self, idx, preds, batch):
        ...
        if len(outputs) > 0:
            for i, output in enumerate(outputs):
                object_id = output[-1]
                action_name = self.model.names[object_id]  # Obtener el nombre de la acción
                # Suponiendo que quieras minutos y segundos
